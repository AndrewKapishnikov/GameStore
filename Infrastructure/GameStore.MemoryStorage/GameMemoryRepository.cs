...
namespace GameStore.MemoryStorage
{
    ...
    {
        private readonly GameMemoryEntity[] games;
        ...
        private void InitialiseGamesData(ref GameMemoryEntity[] games)
        {
            games = new[]
            {
                new GameMemoryEntity(1, "Grand Theft Auto V [GTA 5]", "Rockstar",
                ...
                new GameMemoryEntity(2,"FIFA 21","Electronic Arts",
                ...
                new GameMemoryEntity(3, "Fallout 76", "Bethesda Softworks",
                ...
                new GameMemoryEntity(4, "Wasteland 3","inXile Entertainment",
        ...
        public GameMemoryEntity[] GetAllByCategory(string category)
        {
            
            return games.Where(game => game.Category.Contains(category)).ToArray();
        }

        public GameMemoryEntity[] GetAllByNameOrPublisher(string nameOrPublisher)
        {
            return nameOrPublisher != null ? games.Where(game => game.Name.ToLower().Contains((nameOrPublisher).ToLower())
                                       || game.Publisher.ToLower().Contains((nameOrPublisher).ToLower())).ToArray() : new GameMemoryEntity[0];
        }

        public GameMemoryEntity GetGameById(int id)
        {
            return games.Single(game => game.Id == id);
        }

        public GameMemoryEntity[] GetGamesByIds(IEnumerable<int> gamesId)
        {
            return games.Join(gamesId, game => game.Id, id => id, (game,id) => game).ToArray();
        }

        public GameMemoryEntity[] GetLastSixGameByDataAdding()
        {
